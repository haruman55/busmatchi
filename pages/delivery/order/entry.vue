      ...
      <v-row no-gutters>
        <v-col>
          <v-card-text class="font-weight-bold text-h5">
            ...
            {{ $Const.ORDER_STATUS_DISP[orderInfo.state]?.text ?? '' }}
          </v-card-text>
        </v-col>
      </v-row>
    </v-container>
...
v-if="orderInfo.state == $Const.STATUS_REQUEST || orderInfo.state == $Const.STATUS_UNDERTAKE"
                rounded dark size="x-large" color="yellow" class="mb-2 pr-8 pl-8" @click="entry">
                配車情報を登録する
              </v-btn>
              ...
              <v-table class="table-border">
                <tbody>
                  <tr>
                    ...
                    <td>名称</td>
                    <td class="text-h5" colspan="5">{{ orderDeliveryUserInfo.companyName }} {{
                      orderDeliveryUserInfo.companyTel
                    }}</td>
                    <!-- <td>ご担当者様</td>
...
v-model="paymentDueDate" :teleport="true" locale="jp" auto-apply
    ...
    <v-container class="align-center" fluid>
      <v-row justify="center" no-gutters>
        ...
        <v-col align="center">
          <v-btn
v-if="orderInfo.state == $Const.STATUS_REQUEST" rounded dark size="x-large" color="red"
